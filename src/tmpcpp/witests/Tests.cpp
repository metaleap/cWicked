...
wi::SpriteFont b_a, b_b, b_x, b_y, // Action Buttons: A, B, X, Y
...
void TestsRenderer::Load() {
  ...
  volume.OnSlide([](wi::gui::EventArgs args) { wi::audio::SetVolume(args.fValue / 100.0f, &soundinstance); });
...
void TestsRenderer::Update(float dt) {
  ...
  switch (userdata) {
    ...
    case CONTROLLERTEST: {
      ...
      s_l.text = text_head_axes[2] + std::to_wstring(wi::input::GetAnalog(wi::input::GAMEPAD_ANALOG_THUMBSTICK_L).x) + L", "
                 + std::to_wstring(wi::input::GetAnalog(wi::input::GAMEPAD_ANALOG_THUMBSTICK_L).y);
      s_r.text = text_head_axes[3] + std::to_wstring(wi::input::GetAnalog(wi::input::GAMEPAD_ANALOG_THUMBSTICK_R).x) + L", "
      ...
      static std::wstring color_sets_desc[] = {L"Controller LED: None", L"Controller LED: Cyan", L"Controller LED: Red", L"Controller LED: Green",
                                               L"Controller LED: Yellow"};

      if (wi::input::Down(wi::input::GAMEPAD_BUTTON_2))
        vibration_multipllier = 1.f;
      else
        vibration_multipllier = 0.f;
      if (wi::input::Press(wi::input::GAMEPAD_BUTTON_3)) {
        color_index_set++;
        if (color_index_set > 4)
          color_index_set = 0;
      }
      ...
      wi::jobsystem::Dispatch(ctx, (uint32_t)scene->transforms.GetCount(), 1024, [&](wi::jobsystem::JobArgs args) {
        ...
        XMStoreFloat4x4(&transform.world,
                        transform.GetLocalMatrix()
                            * XMMatrixTranslation(0, std::sin(sec + 20 * (float)args.jobIndex / (float)scene->transforms.GetCount()) * 0.1f, 0));
      });
...
void TestsRenderer::RunJobSystemTest() {
  ...
  // Dispatch test:
  {
    wi::vector<wi::scene::CameraComponent> dataSet(itemCount);
    timer.record();
    wi::jobsystem::Dispatch(ctx, itemCount, 1000, [&](wi::jobsystem::JobArgs args) { dataSet[args.jobIndex].UpdateCamera(); });
    wi::jobsystem::Wait(ctx);
    double time = timer.elapsed();
...
void TestsRenderer::RunSpriteTest() {
  ...
  {
    static wi::Sprite sprite("images/movingtex.png", CONTENT_DIR
                             "logo_small.png"); // first param is the texture we will display (and also scroll here). Second param is a mask texture
    ...
    sprite.anim.movingTexAnim.speedX = 0;
    sprite.anim.movingTexAnim.speedY = 2; // scroll the texture vertically. This value is pixels/second. So because our texture here is 1x2 pixels, just
                                          // scroll it once fully per second with a value of 2
    AddSprite(&sprite);
  ...
  // Spritesheet, single vertical line:
  {
    ...
    sprite.anim.drawRectAnim.horizontalFrameCount =
        1; // ...but this time, limit the horizontal frame count. This way, we can get it to only animate vertically
    AddSprite(&sprite);
