...
void ThirdPersonCamera::update(bool debugDraws) {
  ...
  //   auto ray        = wi::primitive::Ray(vec4From(pos_target.x, pos_target.y, pos_target.z, 0), XMVector3Normalize(target_to_corner), 0,
  //   tmax); auto coll_layer = ~(Layers::Player | Layers::Npc); auto intersects = scene.Intersects(ray,
  //   wi::enums::FILTER::FILTER_NAVIGATION_MESH | wi::enums::FILTER::FILTER_COLLIDER, coll_layer); if (intersects.entity !=
  //   wi::ecs::INVALID_ENTITY) {   // hit something, see if it is between the player and camera:
  //     auto coll_diff
  ...
  //       if (debugDraws)
  //         wi::renderer::DrawPoint(wi::renderer::RenderablePoint {.position = intersects.position, .size = 0.1f, .color = XMFLOAT4(1, 0, 0,
  //         1)});
  //     }
  //   }
  // }
